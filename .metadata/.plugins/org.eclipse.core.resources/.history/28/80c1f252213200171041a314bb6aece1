  ...
  public static void main(String args[]) throws FileNotFoundException {
	...
	for(int j=0;j<numOpera;j++){
		...
		if(opcion.compareTo("ejecutar")==0){
			...
			System.out.println(linea+" "+maxheap.consultar().id);
			linea++;
			maxheap.extraer();
			
		}else if(opcion.compareTo("priorizar")==0){
			prior=maxheap.priorizar();
			//maxheap.datos.
			...
			if (mismo==false){
				tempPro=new Proceso(Integer.parseInt(datos[1]),prior+1);
			}else{
				tempPro=new Proceso(Integer.parseInt(datos[1]),prior);
			}
